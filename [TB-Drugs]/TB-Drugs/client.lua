...
RegisterNetEvent('TB-Drugs:outlawNotify')
AddEventHandler('TB-Drugs:outlawNotify', function(alert)
...
-- Usable Item Event:
RegisterNetEvent("TB-Drugs:UsableItem")
AddEventHandler("TB-Drugs:UsableItem",function()
	local player = PlayerPedId()
	if IsPedInAnyVehicle(player) then
		exports['TB-ProgressBars']:startUI(4000, "CONNECTING USB TO DEVICE")
	else
...
function ChooseDrugMenu()
	...
	function(data, menu)
			...
			if data.current.value == "cancel_drug_job" then
				...
				TriggerServerEvent("TB-Drugs:GetSelectedJob",data.current.value, data.current.BuyPrice, data.current.MinReward, data.current.MaxReward )
				Citizen.Wait(100)
				menu.close()
				ClearPedTasks(player)
				FreezeEntityPosition(player,false)
...
RegisterNetEvent("TB-Drugs:BrowseAvailableJobs")
AddEventHandler("TB-Drugs:BrowseAvailableJobs",function(spot,drugType,minReward,maxReward)
	...
	if currentID == 100 then
		...
		SelectedID = id
		TriggerEvent("TB-Drugs:startMainEvent",id,drugType,minReward,maxReward)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
	end
end)

-- Core Mission Part
RegisterNetEvent('TB-Drugs:startMainEvent')
AddEventHandler('TB-Drugs:startMainEvent', function(id,drugType,minReward,maxReward)
	local Goons = {}
	...
	TriggerServerEvent("TB-Drugs:syncJobsData",Config.Jobs)
	Citizen.Wait(500)
	...
	while not JobCompleted and not StopTheJob do
		...
		if Config.Jobs[id].InProgress == true then
			...
			if NearOtherVehicle == true and drugBoxInHand and IsControlJustPressed(0, 38) then
				...
				TriggerServerEvent("TB-Drugs:JobReward",minRewardD,maxRewardD,typeDrug)
				drugsTaken = true
				StopTheJob = true
			end
...
function LockpickJobVan(selectedJob)
	...
	if Config.PoliceNotfiyEnabled == true then
		TriggerServerEvent('TB-Drugs:DrugJobInProgress',GetEntityCoords(PlayerPedId()),streetName)
	end
	...
	exports['TB-ProgressBars']:startUI(7500, "LOCKPICKING VAN")
	Citizen.Wait(7500)
...
-- Drug Effects Event:
RegisterNetEvent("TB-Drugs:DrugEffects")
AddEventHandler("TB-Drugs:DrugEffects", function(k,v)
    local playerPed = PlayerId()
	local ped = GetPlayerPed(-1)
	if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
		TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_SMOKING_POT", 0, true)
		exports['TB-ProgressBars']:startUI(v.UsableTime, v.ProgressBarText)
...
-- Convert Item Event:
RegisterNetEvent("TB-Drugs:ConvertProcess")
AddEventHandler("TB-Drugs:ConvertProcess", function(k,v)
	...
	if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
		...
		exports['TB-ProgressBars']:startUI(v.ConversionTime, v.ProgressBarText)
		...
		repeat
			...
			if distance < 2 and CanSellToPed(ped) and canSellDrugs and not IsPedInAnyVehicle(player, true) then
				...
				if IsControlJustPressed(1,74) then
					...
					exports['TB-ProgressBars']:startUI((Config.SellDrugsTime * 1000), Config.SellDrugsBarText)
					...
					if chance == 1 or chance == 2 then
						...
						TriggerServerEvent("TB-Drugs:sellDrugs")
					else
						chance = math.random(1,Config.CallPoliceChance)
						if chance == 1 then
							if Config.PoliceNotfiyEnabled == true then
								TriggerServerEvent('TB-Drugs:DrugSaleInProgress',GetEntityCoords(PlayerPedId()),streetName)
							end
...
Citizen.CreateThread(function()
	while true do
		TriggerServerEvent("TB-Drugs:canSellDrugs")
		Citizen.Wait(Config.DrugSaleCooldown * 1000)
	end
end)
...
RegisterNetEvent("TB-Drugs:canSellDrugs")
AddEventHandler("TB-Drugs:canSellDrugs", function(soldAmount)
...
RegisterNetEvent('TB-Drugs:OutlawBlipEvent')
AddEventHandler('TB-Drugs:OutlawBlipEvent', function(targetCoords)
...
RegisterNetEvent("TB-Drugs:syncJobsData")
