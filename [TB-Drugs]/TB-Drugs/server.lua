...
RegisterServerEvent("TB-Drugs:syncJobsData")
AddEventHandler("TB-Drugs:syncJobsData",function(data)
	TriggerClientEvent("TB-Drugs:syncJobsData",-1,data)
end)

-- Server side table, to store cooldown for players:
RegisterServerEvent("TB-Drugs:addCooldownToSource")
AddEventHandler("TB-Drugs:addCooldownToSource",function(source)
...
RegisterServerEvent("TB-Drugs:addConvertingTimer")
AddEventHandler("TB-Drugs:addConvertingTimer",function(source,timer)
...
RegisterServerEvent("TB-Drugs:addDrugEffectTimer")
AddEventHandler("TB-Drugs:addDrugEffectTimer",function(source,timer)
...
ESX.RegisterUsableItem('drugItem', function(source)
	...
	if not HasCooldown(GetPlayerIdentifier(source)) then
		if xPlayer.getInventoryItem(Config.HackerDevice).count >= 1 then
			TriggerClientEvent("TB-Drugs:UsableItem",source)
		else
...
RegisterServerEvent("TB-Drugs:GetSelectedJob")
AddEventHandler("TB-Drugs:GetSelectedJob", function(drugType,BuyPrice,minReward,maxReward)
	local xPlayer = ESX.GetPlayerFromId(source)
	local itemLabel = ESX.GetItemLabel(itemName)
	if xPlayer.getMoney() >= BuyPrice then
		xPlayer.removeMoney(BuyPrice)
		TriggerEvent("TB-Drugs:addCooldownToSource",source)
		TriggerClientEvent("TB-Drugs:BrowseAvailableJobs",source, 0, drugType, minReward, maxReward)
...
RegisterServerEvent("TB-Drugs:JobReward")
AddEventHandler("TB-Drugs:JobReward",function(minReward,maxReward,typeDrug)
	...
	for k,v in pairs(Config.DrugEffects) do 
		ESX.RegisterUsableItem(v.UsableItem, function(source)
			...
			if not DrugEffect(GetPlayerIdentifier(source)) then
				TriggerEvent("TB-Drugs:addDrugEffectTimer",source,v.UsableTime)
				xPlayer.removeInventoryItem(v.UsableItem,1)
				TriggerClientEvent("TB-Drugs:DrugEffects",source,k,v)
			else
		...
		ESX.RegisterUsableItem(v.UsableItem, function(source)
			...
			if xPlayer.getInventoryItem(v.RewardItem).count <= v.MaxRewardItemInv.f or (not scale and xPlayer.getInventoryItem(v.RewardItem).count <= v.MaxRewardItemInv.e) then
				if not Converting(GetPlayerIdentifier(source)) then
					TriggerEvent("TB-Drugs:addConvertingTimer",source,v.ConversionTime)
					xPlayer.removeInventoryItem(v.UsableItem,1)
					xPlayer.removeInventoryItem(v.RequiredItem,requiredItems)
					TriggerClientEvent("TB-Drugs:ConvertProcess",source,k,v)
...
RegisterServerEvent('TB-Drugs:DrugJobInProgress')
AddEventHandler('TB-Drugs:DrugJobInProgress', function(targetCoords, streetName)
	TriggerClientEvent('TB-Drugs:outlawNotify', -1,string.format("^0Shots fired and ongoing grand theft auto at ^5%s^0",streetName))
	TriggerClientEvent('TB-Drugs:OutlawBlipEvent', -1, targetCoords)
end)

RegisterServerEvent('TB-Drugs:DrugSaleInProgress')
AddEventHandler('TB-Drugs:DrugSaleInProgress', function(targetCoords, streetName)
	TriggerClientEvent('TB-Drugs:outlawNotify', -1,string.format("^0Possible drug sale at ^5%s^0",streetName))
	TriggerClientEvent('TB-Drugs:OutlawBlipEvent', -1, targetCoords)
end)

RegisterServerEvent("TB-Drugs:sellDrugs")
AddEventHandler("TB-Drugs:sellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("TB-Drugs:canSellDrugs")
AddEventHandler("TB-Drugs:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		...
		TriggerClientEvent("TB-Drugs:canSellDrugs",source,soldAmount)
	end
end)
