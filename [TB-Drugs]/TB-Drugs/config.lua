...
Config.ListOfDrugs = {
	...
	{ drug = 'jinjag', label = 'Jinjag', Enabled = true, BuyPrice = 50000, MinReward = 4, MaxReward = 10 },
}
...
Config.WeedDrug = "weed"						-- item name in database
Config.JinjagDrug = "jinjag"				    -- item name in database
...
Config.DrugEffects = {
	{ 
		UsableItem 				= "coke",						-- item name in database for usable item
		ProgressBarText			= "SMOKING CRACK COCAINE",		-- progress bar text
		UsableTime 				= 7000,							-- Smoking time in MS
		EffectDuration 			= 1.7,							-- Duration for effect in seconds
		...
		AddArmorValue			= 30,							-- Set value for body armor thats going to be added
	...
	{ 
		UsableItem 				= "weed",						-- item name in database for usable item
		ProgressBarText			= "SMOKING WEED",				-- progress bar text
		UsableTime 				= 7000,							-- item name in database for usable item
		EffectDuration 			= 1.7,							-- Duration for effect in seconds
		FasterSprint 			= false,						-- Enable or disable faster sprint while on drugs
		SprintValue 			= 1.2,							-- 1.0 is default
		MotionBlur 				= true,							-- Enable or disable motion blur while on drugs
	...
	{ 
		...
		ProgressBarText			= "USING HEROIN",				-- progress bar text
		UsableTime 				= 3000,							-- item name in database for usable item
		EffectDuration 			= 5,							-- Duration for effect in seconds
	...
	{ 
		UsableItem 				= "jinjag",					-- item name in database for usable item
		ProgressBarText			= "ABSORBING JINJA GOLD",				-- progress bar text
		UsableTime 				= 5000,							-- item name in database for usable item
		EffectDuration 			= 4,							-- Duration for effect in seconds
		FasterSprint 			= true,						-- Enable or disable faster sprint while on drugs
...
Config.DrugConversion = {
	{ 
		UsableItem 				= "cocaine_packaged",					-- item name in database for usable item
	...
	{ 
		UsableItem 				= "meth_packaged",					-- item name in database for usable item
	...
	{ 
		UsableItem 				= "weed_packaged",					-- item name in database for usable item
	...
	{ 
		...
		RewardItem 				= "coke",						-- item name in database for required item
		RewardAmount 			= {a = 6, b = 10},				-- Amount of RewardItem player receives. "a" is without scale and "b" is with scale
		MaxRewardItemInv		= {e = 44, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "drugbags",					-- item name in database for required item
		RequiredItemAmount 		= {c = 6, d = 10},				-- Amount of RequiredItem for conversion. "c" is without scale and "d" is with scale
	...
	{ 
		UsableItem 				= "jinja",					-- item name in database for usable item
		RewardItem 				= "jinjag",					-- item name in database for required item
		RewardAmount 			= {a = 3, b = 10},				-- Amount of RewardItem player receives. "a" is without scale and "b" is with scale
		MaxRewardItemInv		= {e = 30, f = 40},				-- Amount of RewardItem player can hold in inventory. "e" is without scale and "f" is with scale
		RequiredItem 			= "jinjagcube",					-- item name in database for required item
		RequiredItemAmount 		= {c = 4, d = 10},				-- Amount of RequiredItem for conversion. "c" is without scale and "d" is with scale
		HighQualityScale		= true,							-- enable/disable scale feature for the drugType
		hqscale					= "hqscale",					-- item name in database for hiqh quality scale item
		ProgressBarText			= "JINJA > JINJA GOLD",		-- progress bar text
		ConversionTime			= 6000,							-- set conversion time in MS.
	},
	{ 
	...
	{ 
		...
		RewardItem 				= "weed",						-- item name in database for required item
